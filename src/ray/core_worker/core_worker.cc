...
Status CoreWorker::PutInLocalPlasmaStore(const RayObject &object,
  ...
  // RAY_LOG(DEBUG) << "hucc time for put from plasma total time: " << te_put_obj_plasma << ", " << ts_put_obj_plasma <<"\n";
...
Status CoreWorker::CreateOwnedAndIncrementLocalRef(
  ...
  *object_id = ObjectID::FromIndex(worker_context_.GetCurrentInternalTaskId(),
  ...
  if (owned_by_us) {
    ...
    RAY_LOG(INFO) << "hucc rayIDLE CreateOwnedAndIncrementLocalRef object_id: " << *object_id << "\n";
...
Status CoreWorker::Get(const std::vector<ObjectID> &ids,
  ...
  if (!memory_object_ids.empty()) {
    ...
    if(ids.size()>0) {
      for (int i = 0;i< ids.size();++i)
        RAY_LOG(ERROR) << "memory out raylet client send 0 " << " " << te_get_obj_tmem << " " << ids[i];
    }
  ...
  if(ids.size()>0) {
    for (int i = 0;i< ids.size();++i)
      RAY_LOG(ERROR) << " raylet client send 4 " << worker_context_.GetWorkerID() << " " << te_get_obj_cw << " "  << ids[i];
  }
...
std::vector<rpc::ObjectReference> CoreWorker::SubmitTask(
  ...
  RAY_LOG(DEBUG) << "hucc push normal task id start: " << task_id << " push task time and exec: " << ts_push_task << "\n";
...
Status CoreWorker::ExecuteTask(
  ...
  RAY_LOG(DEBUG) << "hucc time for exec task args: " << te_exec_task_args << ", " << ts_exec_task_args <<"\n"; 
  ...
  RAY_LOG(DEBUG) << "hucc time for exec task callback to lanaguage time: " << te_exec_call_task << ", " << ts_exec_call_task <<"\n"; 
...
bool CoreWorker::PinExistingReturnObject(const ObjectID &return_id,
  ...
  // value from plasma.
  RAY_LOG(DEBUG) << "hucc rayIDLE AddLocalReference PinExistingReturnObject" << "\n";
...
ObjectID CoreWorker::AllocateDynamicReturnId() {
  ...
  RAY_LOG(DEBUG) << "hucc rayIDLE AllocateDynamicReturnId" << "\n";
...
Status CoreWorker::GetAndPinArgsForExecutor(const TaskSpecification &task,
  ...
  for (size_t i = 0; i < task.NumArgs(); ++i) {
    if (task.ArgByRef(i)) {
      ...
      RAY_LOG(DEBUG) << "hucc rayIDLE other Incrementing ref for borrowed ID task: " << task.TaskId() << "job id: " << task.JobId() << "\n";
      ...
      for (const auto &inlined_ref : task.ArgInlinedRefs(i)) {
        ...
        RAY_LOG(DEBUG) << "hucc rayIDLE Incrementing ref for borrowed ID task: " << task.TaskId() << "job id: " << task.JobId() << "\n";
...
void CoreWorker::HandleExit(const rpc::ExitRequest &request,
  ...
  RAY_LOG(DEBUG) << "hucc rayIDLE worker is_idle: " << is_idle << "own_objects: " << own_objects << "pins_in_flight: " << pins_in_flight << "\n";
