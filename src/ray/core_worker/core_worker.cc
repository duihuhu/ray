...
Status CoreWorker::Get(const std::vector<ObjectID> &ids,
  ...
  if (!memory_object_ids.empty()) {
    ...
    RAY_LOG(WARNING) << "hucc time for get from memory total time: " << te_get_obj_tmem << ", " << ts_get_obj_tmem << " " << ids[0] << " " <<"\n" ;
  }
...
Status CoreWorker::SealReturnObject(const ObjectID &return_id,
  ...
  RAY_LOG(ERROR) << "Sealing return object " << return_id << " " << return_object->GetData() != nullptr << " "<< return_object->GetData()->IsPlasmaBuffer();
