...
bool TaskManager::HandleTaskReturn(const ObjectID &object_id,
  ...
  RAY_LOG(DEBUG) << "Task return object " << object_id << " has size "
                 << return_object.size();
  // auto ts_object_task_return = current_sys_time_us();
  // RAY_LOG(ERROR) << "Task return object " << ts_object_task_return << " " << object_id;
  ...
  if (return_object.in_plasma()) {
    ...
    RAY_LOG(WARNING) << "Task return object in plasma " << object_id << " has size "
            << return_object.size() << " " << worker_raylet_id << " " << worker_ip_address << " " << return_object.owner_ip_address();
  } else {
...
void TaskManager::CompletePendingTask(const TaskID &task_id,
  ...
  RAY_LOG(WARNING) << "Completing task " << task_id << " " << worker_addr.ip_address() ;
