...
const LocalObject *ObjectLifecycleManager::CreateObjectInternal(
  ...
  for (int num_tries = 0; num_tries <= 10; num_tries++) {
    ...
    if (result != nullptr) {
      RAY_LOG(DEBUG) << "CreateObjectInternal object info  " << object_info.object_id << " " << result->GetAllocation().address << " " << object_info.data_size;
      return result;
    }
...
void ObjectLifecycleManager::InsertObjectInfo(const absl::optional<Allocation>& allocation , const ray::ObjectInfo &object_info) {
  object_store_->InsertObjectInfo(allocation, object_info);
}

void ObjectLifecycleManager::InsertObjectInfoThread(const Allocation& allocation , const ray::ObjectInfo &object_info, const std::pair<const plasma::LocalObject *, plasma::flatbuf::PlasmaError>& pair) {
