...
struct MetaInfo {
  ...
  // MetaInfo(){}
  MetaInfo(const ray::ObjectID &object_id, const plasma::Allocation &allocation, const ray::ObjectInfo &object_info, size_t export_desc_len=0, size_t ip_address_len=0) :object_id(object_id), allocation(allocation), \
...
// int PushMetaToDpu(const char * server_name, struct doca_comm_channel_ep_t *ep, struct doca_comm_channel_addr_t *peer_addr) {
  ...
  for (auto &entry : *plasma_meta) {
    ...
    // printf("entry.second %x\n", entry.second->GetAllocation().address);
    if (  entry.second->GetAllocation().address == NULL ) {
      continue;
    }
