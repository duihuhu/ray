...
rep_pci_addr_callback(void *param, void *config)
{
	...
	if (cfg->mode == DMA_COPY_MODE_DPU) {
		if (strnlen(rep_pci_addr, PCI_ADDR_LEN) == PCI_ADDR_LEN) {
			DOCA_LOG_ERR("Comm Channel DOCA device PCI address exceeding the maximum size of %d",
				     PCI_ADDR_LEN - 1);
			return DOCA_ERROR_INVALID_VALUE;
		}
...
host_export_memory_map_to_dpu(struct core_state *core_state, struct doca_comm_channel_ep_t *ep,
			      struct doca_comm_channel_addr_t **peer_addr, char **export_desc)
{
	doca_error_t result;
...
memory_alloc_and_populate(struct core_state *core_state, size_t buffer_len, char **buffer)
{
	...
	// // result = doca_mmap_populate(core_state->mmap, *buffer, buffer_len, sysconf(_SC_PAGESIZE), NULL, NULL);
...
dpu_cleanup_core_objs(struct core_state *state)
{
	...
	result = doca_mmap_dev_rm(state->mmap, state->dev);
	if (result != DOCA_SUCCESS)
		DOCA_LOG_ERR("Failed to remove device from mmap: %s", doca_get_error_string(result));
}
	...
	struct doca_dev_rep **dev_rep)
{
	...
	result = parse_pci_addr(cfg->cc_dev_pci_addr, &pci_bdf);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to parse Comm Channel DOCA device PCI address");
		doca_comm_channel_ep_destroy(*ep);
		return result;
	}

	result = open_doca_device_with_pci(&pci_bdf, NULL, dev);
	if (result != DOCA_SUCCESS) {
	...
	if (cfg->mode == DMA_COPY_MODE_DPU) {
		result = parse_pci_addr(cfg->cc_dev_rep_pci_addr, &pci_bdf);
		if (result != DOCA_SUCCESS) {
			DOCA_LOG_ERR("Failed to parse Comm Channel DOCA representor device PCI address");
			doca_comm_channel_ep_destroy(*ep);
			return result;
		}

		result = open_doca_device_rep_with_pci(*dev, DOCA_DEV_REP_FILTER_NET, &pci_bdf, dev_rep);
		if (result != DOCA_SUCCESS) {
...
create_core_objs(struct core_state *state, enum dma_copy_mode mode)
{
  ...
  return DOCA_SUCCESS;
	...
	// result = doca_mmap_set_max_num_chunks(state->mmap, max_chunks);
	// if (result != DOCA_SUCCESS)
	// 	DOCA_LOG_ERR("Unable to set memory map nb chunks: %s", doca_get_error_string(result));
		    ...
		    struct doca_comm_channel_addr_t **peer_addr, const plasma::Allocation &allocation)
{
  ...
  result = doca_mmap_populate(core_state->mmap, allocation.address, allocation.size, sysconf(_SC_PAGESIZE), NULL, NULL);
	...
	size_t export_desc_len;
	/* Export memory map to allow access to this memory region from DPU */
	result = doca_mmap_export(core_state->mmap, core_state->dev, (void **)allocation.address, &allocation.size);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to export DOCA mmap: %s", doca_get_error_string(result));
		return result;
	}
	...
	std::cout<< "Final status message was successfully received" <<std::endl;
