...
struct doca_dpi_config_t {
	...
	/**< The maximum length that DPI guarantee to provide a match on, including across consecutive packets.
	  *  Must be <= 5000
	  *  For example:
	  *        Signature = A.*B
	  *        max_sig_match_len = 5
	  *  DPI guarantee that AAAAB will be found (len <= 5)
	  *  DPI does not guarantee that AAAAAAAAAAAAB will be found (len > 5)
	  */
};
