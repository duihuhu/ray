...
Status PlasmaClient::Impl::GetBuffers(
  ...
  // RAY_LOG(ERROR) << "hucc get remote plasma plasma: " << te_get_remote_plasma << ", " << ts_get_remote_plasma << "," << te_get_remote_plasma - ts_get_remote_plasma << "\n";
...
Status PlasmaClient::Impl::GetObjectMeta(const ObjectID &object_id) {
  ...
  RAY_LOG(DEBUG) << "ReadMetaReply GetObjectMeta " << object_id << " address " << address << " object_size " << object_size << " device_num " <<  device_num << " virt_address " << virt_address;
