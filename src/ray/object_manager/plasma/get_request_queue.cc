...
void GetRequestQueue::AddRequest(const std::shared_ptr<ClientInterface> &client,
  ...
  for (const auto &object_id : unique_ids) {
    ...
    if (entry && entry->Sealed()) {
      ...
      RAY_LOG(DEBUG) << " get_requests_ object info " << get_request->objects[object_id].store_fd << " " << get_request->objects[object_id].data_offset << " " <<  get_request->objects[object_id].data_size << " " << get_request->objects[object_id].metadata_size; 
