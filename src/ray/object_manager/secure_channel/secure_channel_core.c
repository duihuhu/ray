...
rep_pci_addr_callback(void *param, void *config)
{
	...
	if (cfg->mode == SC_MODE_DPU) {
		if (strnlen(rep_pci_addr, PCI_ADDR_LEN) == PCI_ADDR_LEN) {
			DOCA_LOG_ERR(
				"Comm Channel DOCA device representor PCI address exceeding the maximum size of %d",
				PCI_ADDR_LEN - 1);
			return DOCA_ERROR_INVALID_VALUE;
		}
...
init_cc(struct sc_config *cfg, struct cc_ctx *ctx, struct doca_dev **dev, struct doca_dev_rep **dev_rep)
{
	...
	if (cfg->mode == SC_MODE_DPU) {
		...
		if (result != DOCA_SUCCESS) {
			DOCA_LOG_ERR("Failed to parse Comm Channel DOCA device representor PCI address");
			doca_comm_channel_ep_destroy(ctx->ep);
			return result;
		}
...
register_secure_channel_params()
{
	...
	doca_argp_param_set_short_name(pci_addr_param, "d");
	doca_argp_param_set_long_name(pci_addr_param, "dev-pci");
