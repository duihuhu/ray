...
uint64_t ObjectManager::Pull(const std::vector<rpc::ObjectReference> &object_refs,
                             BundlePriority prio) {
  RAY_LOG(DEBUG) << "object manager pull";
        ...
        [this, object_id, client_id, rpc_client]() {
          ...
          rpc_client->Pull(
              ...
              [object_id, client_id](const Status &status, const rpc::PullReply &reply) {
                ...
                RAY_LOG(WARNING) << " hucc remote get object receive pull reply " << " object id " << object_id << " " << te_receive_pull_reply << "\n";
              });
        },
...
void ObjectManager::HandleSendFinished(const ObjectID &object_id,
  ...
  RAY_LOG(DEBUG) << "hucc HandleSendFinished on " << self_node_id_ << " to " << node_id
                 << " of object " << object_id << " chunk " << chunk_index
                 << ", status: " << status.ToString() << "start_time: " << str_start_time << " end_time: " << str_end_time << "\n";
  if (!status.ok()) {
...
void ObjectManager::SendObjectChunk(const UniqueID &push_id,
  ...
  rpc::ClientCallback<rpc::PushReply> callback =
      [this, start_time, object_id, node_id, chunk_index, on_complete](
        ...
        RAY_LOG(DEBUG) << "hucc remote get object receive handle push reply object id " << object_id << " " << te_handle_push_request_reply  << " chunk_index: " << chunk_index << "\n";
  ...
  RAY_LOG(DEBUG) << "hucc remote get object send push request object id :" << object_id << " " << ts_push_request << " chunk_index: " << chunk_index <<"\n";
...
void ObjectManager::HandlePush(const rpc::PushRequest &request,
  ...
  //hucc breakdown get object handle push
  // RAY_LOG(DEBUG) << "hucc breakdown get object handle push: " << object_id  << " " << ts_breakdown_handle_push << "\n";
  //end hucc 
  ...
  RAY_LOG(DEBUG) << "hucc breakdown get object write to plasma start: " << object_id  << " " << ts_breakdown_write_plasma << " chunk_index: " << chunk_index << "\n";
  //end hucc 
  ...
  // RAY_LOG(DEBUG) << "hucc breakdown get object write to plasma end: " << object_id  << " " << te_breakdown_write_plasma << "\n";
  //end hucc
  ...
  RAY_LOG(DEBUG) << "hucc remote get object receive handle push request object id " << object_id  << " " << te_handle_push_request << " chunk_index: " << chunk_index <<"\n";
...
void ObjectManager::HandlePull(const rpc::PullRequest &request,
  ...
  RAY_LOG(DEBUG) << "hucc remote get object receive handle pull request from " << node_id << " of object " << object_id << " " << ts_handle_pull_request << "\n";
  main_service_->post([this, object_id, node_id, start_time]() { Push(object_id, node_id, start_time); },
...
std::pair<const plasma::LocalObject *, plasma::flatbuf::PlasmaError> ObjectManager::CreateObjectRdma(const ray::ObjectInfo &object_info) {
...
void ObjectManager::InsertObjectInfoThread(const plasma::Allocation &allocation, const ray::ObjectInfo &object_info, const std::pair<const plasma::LocalObject *, plasma::flatbuf::PlasmaError>& pair) {
  //   HandleObjectAdded(object_info);
