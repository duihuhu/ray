...
void ObjectManager::HandleSendFinished(const ObjectID &object_id,
  ...
  RAY_LOG(WARNING) << "hucc HandleSendFinished on " << self_node_id_ << " to " << node_id
                 << " of object " << object_id << " chunk " << chunk_index
                 << ", status: " << status.ToString() << "start_time: " << str_start_time << " end_time: " << str_end_time << "\n";
  if (!status.ok()) {
...
void ObjectManager::HandlePush(const rpc::PushRequest &request,
  ...
  RAY_LOG(WARNING) << "hucc breakdown get object write to plasma start: " << object_id  << " " << ts_breakdown_write_plasma << " chunk_index: " << chunk_index << "\n";
  //end hucc 
...
std::pair<const plasma::LocalObject *, plasma::flatbuf::PlasmaError>& ObjectManager::CreateObjectRdma(const ray::ObjectInfo &object_info) {
  RAY_LOG(DEBUG) << "before ObjectManager CreateObjectRdma";
  auto pair = plasma::plasma_store_runner->CreateObjectRdma(object_info);
  RAY_LOG(DEBUG) << "after ObjectManager CreateObjectRdma";
  return pair;
}
...
void ObjectManager::InsertObjectInfoThread(const absl::optional<plasma::Allocation> &allocation, const ray::ObjectInfo &object_info, const std::pair<const plasma::LocalObject *, plasma::flatbuf::PlasmaError>& pair) {
  HandleObjectAdded(object_info);
